...
<head>
    ...
    <script>
        function copyFunction() {
            var copyText = document.getElementById("oauth2-token");
            copyText.select();
            copyText.setSelectionRange(0, 99999);
            document.execCommand("copy");

            var tooltip = document.getElementById("cp-tool-tip");
    ...
    <%
        ...
        LoggedInSessionBean.AccessTokenResponseBean tokenResponseBean =  null;
        ClaimManagerProxy claimManagerProxy = (ClaimManagerProxy) application.getAttribute("claimManagerProxyInstance");
        Map<String, String> subjectAttributeDisplayValueMap = claimManagerProxy
                        .getLocalClaimUriDisplayValueMapping(new ArrayList<>(subjectAttributeValueMap.keySet()));
        
        boolean errorOccurred = Boolean.parseBoolean(request.getParameter("error"));
        String errorMsg = null;
        if (!errorOccurred) {
            tokenResponseBean = sessionBean.getSAML2SSO().getAccessTokenResponseBean();
        } else {
                                    ...
                                    <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                                        <a class="nav-item nav-link active" id="nav-overview-tab" data-toggle="tab"
                                           href="#nav-overview" role="tab" aria-controls="nav-overview" aria-selected="true">
                                            <i class="fas fa-list-ul"></i> &nbsp;Token Details</a>
                                    </div>
                                </div>
                            </nav>
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>:</td>
                                                    <td>
                                                        <span class="oauth2-token"><%= tokenResponseBean.getAccessToken()%></span>
                                                        &nbsp;&nbsp;
                                                        <p class="copied token-copied">Copied..!</p>
                                                        <button class="btn btn-primary btn-clipboard clipboard-token"
                                                                data-clipboard-target=".oauth2-token"><i
                                                ...
                                                <tr>
                                                    <td>Expires in</td>
                                                    <td>:</td>
                                                    <td><%= tokenResponseBean.getExpiresIn()%></td>
                                                </tr>
                                                <tr>
                                                    <td>Token Type</td>
                                                    <td>:</td>
                                                    <td><%= tokenResponseBean.getTokenType()%></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                        <%  } else {%>
                                    ...
                                    <div class="request-response-details mt-3">
                                        ...
                                        <div class="code-container mb-3">
                                            <button class="btn btn-primary btn-clipboard"
                                                    ...
                                                    class="fa fa-clipboard"></i></button>
                                            <p class="copied">Copied..!</p>
                                            <pre><code class="copy-target3 JSON pt-3 pb-3 requestContent"><%=tokenResponseBean.toString()%></code></pre>
                                        </div>
                                    </div>
                                </div>
                            </li>
